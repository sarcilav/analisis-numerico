...
struct matriz
{
  ...
  int* x;  //orden de la solucion entregada
  ...
  matriz(int ni,int  mi){
    n = ni;
    m = mi;
    x = new int[mi-1]; //orden de las x en la solucion
...
void piv_total(matriz * in, int k) //k = etapa a la que se le busca el mayor
{
  double mayor = 0.0;
  int fila = k;
  int columna = k;
  for(int i = k; i < in->n; ++i)//se encuentra el mayor de las filas
    {
      //int j = k;
      for(int j = k; j < in->m-1; ++j){//se encuentra el mayor de las columnas
	if(fabs(in->p[i][j]) > mayor)
	  {
	    mayor = fabs(in->p[i][j]);
	    fila = i;
	    columna = j;
  ...
  printf("--------mayor : %lf, de la etapa: %d en la fila: %d y columna: %d\n", mayor, k, fila, columna);
  imprimir(*in);
   if(k != fila)
     {
       for(int i = 0; i < in->m; ++i){//cambio las filas
 	double aux = in->p[k][i];
 	in->p[k][i] = in->p[fila][i];
 	in->p[fila][i] = aux;  
       }
     }
   if(k != columna){ //cambio columnas
     int auxiliar = in->x[k];
     in->x[k] = in->x[columna];
     in->x[columna] = auxiliar; //dejo registro de que cambie campos
     for(int j = 0; j < in->n; ++j){
       double aux = in->p[j][k];
       in->p[j][k] = in->p[j][columna];
       in->p[j][columna] = aux;
     }
   }
...
int main()
{
  ...
  for(int i = 0; i < resultados.m; ++i)
    {
      printf("x%d: %lf ", entrada.x[i], resultados.p[0][i] );
    }
  printf("\n");
